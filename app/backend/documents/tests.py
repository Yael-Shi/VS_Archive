...
class UploadPageTemplateTests(TestCase):
    ...
    def test_upload_page_renders_gallery_first_actions(self):
        ...
        self.assertNotContains(resp, 'id="cameraCaptureBtn"')
        self.assertNotContains(resp, "צילום ישיר (ניסיוני)")
        self.assertNotContains(resp, "upload-file-action--experimental")
    ...
    def test_upload_page_renders_upload_ui_revision_stamp(self):
        resp = self._get_page()
        self.assertEqual(resp.status_code, 200)
    ...
    def test_upload_page_is_not_cached(self):
        resp = self._get_page()
        self.assertEqual(resp.status_code, 200)
    ...
    def test_upload_page_js_wires_gallery_incremental_upload(self):
        ...
        self.assertNotContains(resp, "function startCameraCaptureSession(")
        self.assertNotContains(resp, 'input.setAttribute("capture", "environment")')
        self.assertNotContains(resp, "cameraCaptureBtn")
        self.assertNotContains(resp, "צילום ישיר (ניסיוני)")
        self.assertNotContains(resp, "isMobileUploadUi")

    def test_upload_page_does_not_render_direct_camera_ui(self):
        resp = self._get_page()
        self.assertEqual(resp.status_code, 200)
        self.assertNotContains(resp, 'id="cameraCaptureBtn"')
        self.assertNotContains(resp, "צילום ישיר (ניסיוני)")
        self.assertNotContains(resp, "upload-file-action--experimental")
        self.assertNotContains(resp, "function startCameraCaptureSession(")
        self.assertNotContains(resp, 'setAttribute("capture", "environment")')
        self.assertNotContains(resp, "cameraSessionActive")
        self.assertNotContains(resp, "AWAITING_CAMERA")
        self.assertNotContains(resp, "צילום ישיר מוסיף עמוד אחד בכל פעם")
        self.assertNotContains(resp, "לא מומלץ בנייד; ב-Android Chrome")

    def test_upload_page_still_renders_pdf_upload_controls(self):
        resp = self._get_page()
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "appendStagedPdfFile")
...
class StatusLabelPresentationTests(TestCase):
    ...
    def test_list_page_metadata_status_is_hebrew(self):
        ...
        self.assertNotContains(resp, ">סטטוס<")
...
class DocumentVisibilityAccessControlTests(TestCase):
    ...
    def test_staff_list_api_includes_processing_and_upload_status(self):
        ...
        self.assertEqual(
            item["metadata_status"], public_doc.archive_item.metadata_status
        )
    ...
    def test_anonymous_metadata_status_query_does_not_hide_needs_completion_docs(self):
        public_doc = self._create_document(
            visibility=Document.Visibility.PUBLIC,
            title="Public needs completion stays listed",
            metadata_status=Document.MetadataStatus.NEEDS_COMPLETION,
        )
        resp = self.client.get(
            "/api/documents/",
    ...
    def test_viewer_metadata_status_query_does_not_hide_needs_completion_docs(self):
        public_doc = self._create_document(
            visibility=Document.Visibility.PUBLIC,
            title="Viewer needs completion stays listed",
            metadata_status=Document.MetadataStatus.NEEDS_COMPLETION,
        )
        self.client.force_login(self.viewer)
        resp = self.client.get(
    ...
    def test_staff_metadata_status_completed_filter_still_works(self):
        needs = self._create_document(
            visibility=Document.Visibility.PUBLIC,
            title="Staff needs completion filter doc",
            metadata_status=Document.MetadataStatus.NEEDS_COMPLETION,
        )
        completed = self._create_document(
            visibility=Document.Visibility.PUBLIC,
            title="Staff completed filter doc",
            metadata_status=Document.MetadataStatus.COMPLETED,
        )
        self.client.force_login(self.staff)
        resp = self.client.get(
        ...
        self.assertNotIn(needs.id, ids)

    def test_anonymous_list_page_hides_metadata_status_filter_and_column(self):
        public_doc = self._create_document(
            visibility=Document.Visibility.PUBLIC,
        ...
        self.assertNotContains(resp, 'id="filter-metadata-status"')
        self.assertNotContains(resp, "השלמת פרטים")
        self.assertNotContains(resp, "דרושה השלמת פרטים")
        self.assertNotContains(resp, "פרטים הושלמו")
        self.assertNotContains(resp, "השלמת פרטים:")

    def test_viewer_list_page_hides_metadata_status_filter_and_column(self):
        public_doc = self._create_document(
            visibility=Document.Visibility.PUBLIC,
    ...
    def test_staff_list_page_still_has_upload_status_filter(self):
        self._create_document(
            visibility=Document.Visibility.PUBLIC,
            title="Staff metadata column header doc",
            metadata_status=Document.MetadataStatus.NEEDS_COMPLETION,
        )
        self.client.force_login(self.staff)
        resp = self.client.get("/api/ui/documents/")
