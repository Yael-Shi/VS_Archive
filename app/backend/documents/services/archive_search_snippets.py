...
def build_archive_search_match_presentation(
    *,
    ...
    without fabricating a body excerpt. PHOTO component text and
    ``ArchiveItemPerson`` identities live in ``metadata_text``; when they
    match and no ArchiveItem scalar/M2M field does, the generic item-details
    label is used (no per-photo snippet, Person page, or ``?photo=``).
    """
    if not terms:
        return None
    ...
    if short_field_contains_any_term(index_metadata, terms):
        # PhotoContent / PhotoPerson / ArchiveItemPerson text lives in
        # metadata_text, not on ArchiveItem scalar/M2M fields. Label as item
        # details; do not invent a per-photo snippet, Person page, or
        # ``?photo=`` deep-link.
        return ArchiveSearchMatchPresentation(
