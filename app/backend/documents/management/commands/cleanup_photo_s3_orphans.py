...
from documents.services.photo_s3_orphan_cleanup import (
    ...
    apply_photo_s3_orphan_cleanup,
    build_photo_s3_orphan_cleanup_report,
)


class Command(BaseCommand):
    help = (
        "Audit and optionally delete orphaned S3 objects under photos/ that are "
        "not referenced by PhotoContent.original_file_key or "
        "PhotoContent.thumbnail_file_key. Default is dry-run."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--prefix",
            default="photos/",
            help="S3 prefix to scan (default: photos/). Must stay within photos/.",
        )
        parser.add_argument(
            "--limit",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        try:
            report = build_photo_s3_orphan_cleanup_report(
        ...
        if commit_mode:
            try:
                apply_result = apply_photo_s3_orphan_cleanup(report)
            except ValueError as exc:
                raise CommandError(str(exc)) from exc
    ...
    def _write_candidate_rows(self, report: PhotoS3OrphanCleanupReport) -> None:
        if not report.candidates:
            self.stdout.write("")
            self.stdout.write("No orphaned photo S3 objects found.")
            return
    ...
    def _write_dry_run_output(self, report: PhotoS3OrphanCleanupReport) -> None:
        self.stdout.write("Photo S3 orphan cleanup (dry run)")
        self.stdout.write(
    ...
    def _write_commit_output(
        ...
        apply_result: PhotoS3OrphanCleanupApplyResult,
    ) -> None:
        self.stdout.write("Photo S3 orphan cleanup (commit)")
