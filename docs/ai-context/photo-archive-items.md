...
- **PHOTO detail** (`/archive/<id>/`) — presigned GET for the **selected** photo’s **`original_file_key`** (full original), not the browse thumbnail. `?photo=<photo_content_id>` selects a renderable photo on this item; omitted/invalid/foreign values fall back to the first renderable photo. One renderable photo keeps the simple detail layout. Multiple renderable photos show Previous/Next, thumbnail selectors (stored thumbs or numbered fallback), and per-selected-photo metadata. Shared ArchiveItem metadata is shown once, including structured Author presentation via `source_metadata.html`.
...
- Per-photo identified-person picker labels include aliases for staff
  ...
  **`/archive/manage/people/<person_id>/edit/`** for canonical rename,
  ...
  token creates a new Person as **`PhotoPerson`** only unless an existing
  canonical/alias match requires per-token force-create. Staff can
...
2. **`POST /api/photo-uploads/add/`** validates optional **`person_ids`** / **`new_person_name`** before any create, locks the **`ArchiveItem`**, allocates **`max(position)+1`**, creates pending **`PhotoContent`**, writes **`PhotoPerson`** only for selected and newly created people, and returns the same style of presigned PUT. It does not write **`ArchiveItemPerson`**.
