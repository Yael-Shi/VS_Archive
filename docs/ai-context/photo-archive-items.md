...
- Edit shared metadata at **`/archive/manage/<id>/edit/`**. The same page renders every `PhotoContent` as an independent editable card (thumbnail, status, filename, **צפייה**, reorder, delete, per-photo fields). Shared metadata is not duplicated inside a photo card. A PHOTO item must keep at least one `PhotoContent`. Successful **shared** save stays on **`/archive/manage/<id>/edit/`**. Successful **inline** per-photo save returns to that URL with `#photo-<photo_id>`. Inline validation errors re-render the item edit page (HTTP 200) with errors on that card. **`/archive/manage/<id>/photos/<photo_id>/edit/`** remains a compatibility fallback; a non-inline save still stays on that URL. PHOTO **create** still redirects to **`/archive/manage/`**. Add-photo JS still returns to the item edit page. Reorder and delete-one-photo still use their existing endpoints. The item manage toolbar keeps existing **חזרה לפריט** to **`/archive/<id>/`**. Each card’s **צפייה** goes to **`/archive/<id>/?photo=<photo_id>`**.
