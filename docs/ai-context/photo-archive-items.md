...
**Status:** Design (PR1) through staff manage status clarity (PR6) are **implemented**. **Browse-card thumbnail generation**, **upload-time thumbnail persistence**, and **idempotent backfill commands** are **implemented**. The **multi-photo data model**, **staff multi-photo management (PR3)**, the **public multi-photo gallery (PR4)**, **search aggregation across PhotoContent / PhotoPerson (multi-photo PR5)**, **Person aliases schema + PHOTO search (PR6a)**, and **staff Person/alias management UI (PR6b)** are **implemented**. **Staff Person identity merge** (explicit keeper/duplicate ids; not name matching) is **implemented**. **Public Person page** is **implemented** as one unified DISTINCT ArchiveItem stream from ArchiveItemPerson, renderable PhotoPerson, or AIA via explicit `Author.person` (`/archive/people/<id>/`; PhotoPerson matches deep-link to `?photo=`). **Unified public People directory (PR2)** is **implemented** (`/archive/people/` absorbs linked Authors; Author-only rows remain; `/archive/authors/` redirects). **PHOTO structured ArchiveItem Author staff UX** is **implemented** (same `author_ids` / `new_author_name` create/edit contract as OCR/MANUAL/VIDEO; ArchiveItem-level only; PHOTO detail shows shared source metadata). **Unified staff PHOTO edit cards** (inline per-photo forms on the item edit page, with a staff `?photo=` selector for multi-photo items) are **implemented**. Public alias display, re-upload/retry after **`FAILED`**, and browse-card aggregation remain **not implemented**.
...
- `documents/views.py` — **`/archive/`** list/detail, unified create at **`/archive/manage/new/`**, PHOTO item edit with inline per-photo cards at **`/archive/manage/<id>/edit/`**, staff Person index at **`/archive/manage/people/`**, staff Person edit at **`/archive/manage/people/<id>/edit/`**, staff Person merge confirm at **`/archive/manage/people/<id>/merge/`**
...
- **PHOTO detail** (`/archive/<id>/`) — multi-photo items with no `?photo=` show a responsive grid of authorized/renderable photos (order `(position, id)`); tiles link to `?photo=<photo_content_id>`. Selected photo (or a one-photo item) presigns GET for that photo’s **`original_file_key`**. Per-photo public metadata is rendered **above** the image. Invalid/foreign/non-renderable `?photo=` values fall back to the first renderable photo. Shared ArchiveItem metadata is shown once, including structured Author presentation via `source_metadata.html`. Selected-photo PhotoPerson names are **אנשים בתמונה**. Item-level ArchiveItemPerson is **אנשים קשורים לפריט**, minus `Person.id` values already in the selected-photo list (hidden when empty). Album view keeps the full AIP list under that item heading and does not show PhotoPerson. Dedup is `Person.id` only.
...
**`ArchiveItemPerson` is not a photo appearance.** Item-level person links are indexed as ArchiveItem metadata (canonical name + aliases, all item types) and do not select a photo, generate `?photo=`, or depend on photo renderability. `PhotoPerson` remains the only “this person appears in this photo” relation. **PhotoPerson implies ArchiveItemPerson** (add-only). **ArchiveItemPerson does not imply PhotoPerson.** Ordinary Tags still power tag browse/filter. Mapped historical person-name Tag browse URLs 302 to `/archive/people/<mapped_person_id>/` and those ids are omitted from public Tag choices; raw `?tag=` is unchanged (Option 0; matches nothing after D1 relation cleanup). D1 (`cleanup_historical_person_tags --apply-relations`) removes mapped Tag relations and rebuilds affected search indexes. D2a retired-name policy is implemented. D2b (`delete_historical_person_tag_rows --apply-rows`) deletes the 29 frozen Tag rows; the frozen map remains.

**Staff write path (C1):** ArchiveItem-level people are managed on ArchiveItem **create and edit** for all types, including PHOTO create (`/archive/manage/new/?item_type=photo`) and the PHOTO shared-metadata edit page (`/archive/manage/<id>/edit/`) under **אנשים קשורים לפריט**. PHOTO create writes **`ArchiveItemPerson`** (related to the archival item) and does **not** write **`PhotoPerson`**. Photo-level identified people are **אנשים מזוהים בתמונה** on each photo card of the PHOTO item edit page, on the standalone per-photo fallback editor, and on add-photo (`/archive/manage/<id>/photos/add/`). **`new_archive_item_person_name`** and photo-level **`new_person_name`** accept comma-separated canonical names: split/trim/drop empty/order-preserving dedupe within that input only; each token creates a new Person unless an existing canonical or alias exact (case-insensitive) match requires per-token force-create (warning, not automatic reuse or merge). Item input creates **`ArchiveItemPerson` only**; photo input (edit and add) creates **`PhotoPerson`** and **ensures** matching **`ArchiveItemPerson`** (add-only). Removing PhotoPerson does not delete AIP. Item-level AIP writers never create PhotoPerson. Staff REPLACE (`set_archive_item_people`) keeps every Person still implied by PhotoPerson on that item even if the form omits them; `delete_archive_item_person` is a NOOP while such a PhotoPerson remains. **C2a** stores user-submitted ArchiveItemPerson ADD/REMOVE deltas (`ArchiveItemPersonSuggestion`) and applies them through `create_archive_item_person` / `delete_archive_item_person` only; it never infers PhotoPerson. **C2b** is the public metadata-suggestion Person delta UI plus a dedicated staff Person-suggestion backlog; it does not infer or mutate PhotoPerson. Public cards and non-PHOTO detail show ArchiveItemPerson under **אנשים קשורים** (links to `/archive/people/<id>/`) and hide the 29 frozen historical person-name Tag ids from public discovery. Public PHOTO detail shows selected-photo PhotoPerson under **אנשים בתמונה** and leftover AIP under **אנשים קשורים לפריט** (canonical-name links to the Person page; same `Person.id` is not listed twice). Stage B mapped-tag browse redirects to the Person page and public Tag-choice hiding are implemented; raw `?tag=` is unchanged. D1 mapped relation cleanup is implemented; D2a retired-name policy is implemented; D2b Tag-row deletion is implemented.
...
**Current product/UI rule:** staff can attach **multiple image files** to one PHOTO item. Public **`/archive/<id>/`** for a multi-photo item is an album grid of renderable photos; **`?photo=`** shows one photo with metadata above the image. A single renderable photo skips the grid. Browse cards still use **the first image** (`primary_photo_content`). Public `q` search aggregates text from **public-renderable photos** onto that one ArchiveItem. No OCR text extraction.
...
- Edit shared metadata at **`/archive/manage/<id>/edit/`**. Per-photo metadata is a separate independent form on that page (not mixed into the item POST). Multi-photo items show a compact selector (thumbnail + **תמונה N**, order `(position, id)`) linking to **`?photo=<photo_id>`** and render **only the selected** PhotoContent editor. Absent `?photo=` selects the first photo; invalid/foreign `?photo=` is staff HTTP 404. Single-photo items skip the selector. Shared metadata is not duplicated inside the photo editor. A PHOTO item must keep at least one `PhotoContent`. Successful **shared** save stays on **`/archive/manage/<id>/edit/`**. Successful **inline** per-photo save returns to **`/archive/manage/<id>/edit/?photo=<photo_id>`**. Inline validation errors re-render the item edit page (HTTP 200) with that photo selected. **`/archive/manage/<id>/photos/<photo_id>/edit/`** remains a compatibility fallback; a non-inline save still stays on that URL. PHOTO **create** still redirects to **`/archive/manage/`**. Add-photo JS returns to the item edit page with **`?photo=<new_photo_id>`** when that id is known. Reorder and delete-one-photo still use their existing endpoints and return with a safe remaining/current `?photo=` when available. The item manage toolbar keeps existing **חזרה לפריט** to **`/archive/<id>/`**. The selected card’s **צפייה** goes to **`/archive/<id>/?photo=<photo_id>`**.
- Per-photo file replace is not implemented.
- Per-photo identified-person picker labels include aliases for staff
  (`Canonical (Alias 1, Alias 2)`). The same controls are on **add-photo**.
  Selected people link to
  **`/archive/manage/people/<person_id>/edit/`** for canonical rename,
  ...
  token creates a new Person as **`PhotoPerson`** and ensures matching
  **`ArchiveItemPerson`** unless an existing canonical/alias match requires
  per-token force-create. Staff can
...
| **`/archive/<id>/`** | Multi-photo: album **grid** when `photo` is omitted; selected photo via `?photo=` with per-photo metadata **above** the image and presigned original. Single-photo items skip the grid. Invalid `?photo=` falls back to the first renderable photo. |
...
1. Staff open **`/archive/manage/<id>/photos/add/`** (same PhotoPerson picker/`new_person_name` as per-photo edit).
2. **`POST /api/photo-uploads/add/`** validates optional **`person_ids`** / **`new_person_name`** before any create, locks the **`ArchiveItem`**, allocates **`max(position)+1`**, creates pending **`PhotoContent`**, writes **`PhotoPerson`** for selected and newly created people, **ensures matching ArchiveItemPerson** (add-only), and returns the same style of presigned PUT. It does not create PhotoPerson from AIP and does not delete AIP.
3. Browser PUT + **`/complete/`** are the existing finalize path. Add-photo JS still redirects to the item management page.
...
- staff PHOTO appearance review / PhotoPerson backfill **from `people_present`**; no new-Person proposals on public AIP suggestions. **Reviewed PhotoPerson import v1** (JSON artifact, dry-run / `--apply`, `ReviewedPersonImportBinding` for `create_person` idempotency; ADD ensures AIP; legacy missing AIP is `REPAIR`) is **implemented** as a management command — not a site UI and not a `people_present` parser. Operational **`backfill_archive_item_person_from_photo_person`** backfills LIVE PhotoPerson→AIP gaps (dry-run default).
...
| **Multi-photo PR4** | Public gallery / per-selected-photo metadata / identified Person names (now Person-page links) | **Implemented** |
...
| **C1 ArchiveItemPerson staff UI** | Item-level people manager on ArchiveItem **create and edit** for all types; PHOTO heading distinguished from PhotoPerson; PHOTO create does not write PhotoPerson; public cards/detail cutover is Stage A; Stage B mapped-tag browse redirect / public Tag-choice hiding implemented separately | **Implemented** |
...
| **C2b ArchiveItemPerson suggestion UI** | Public metadata-suggestion page submits Person ADD/REMOVE deltas; dedicated staff backlog + approve/reject; Person.id identity; no new-Person proposals; PhotoPerson remains separate; historical person Tags unchanged on that form; Stage A public cards/detail and Stage B tag browse/choice cutover implemented separately | **Implemented** |
