...
- **PHOTO detail** (`/archive/<id>/`) — multi-photo items with no `?photo=` show a responsive grid of authorized/renderable photos (order `(position, id)`); tiles link to `?photo=<photo_content_id>`. Selected photo (or a one-photo item) presigns GET for that photo’s **`original_file_key`**. Per-photo public metadata is rendered **above** the image. Invalid/foreign/non-renderable `?photo=` values fall back to the first renderable photo. Shared ArchiveItem metadata is shown once, including structured Author presentation via `source_metadata.html`. PhotoPerson names are selected-photo appearance only; ArchiveItemPerson stays under **אנשים קשורים**.
