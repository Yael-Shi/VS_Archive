...
- Eventual person-Tag removal (ordinary Tags still power unmapped tag browse/filter; mapped Tag browse redirects; raw `?tag=` preserved); automatic ArchiveItemPerson from PhotoPerson; staff PHOTO appearance review / PhotoPerson backfill from `people_present`; public Person filter/browse UX; search-index cleanup of mapped Tag names; destructive Tag/relation cleanup; no new-Person proposals
- Public Person pages
