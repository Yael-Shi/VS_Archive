...
- Eventual D2 person-Tag *row* removal (ordinary Tags still power unmapped tag browse/filter; mapped Tag browse redirects; raw `?tag=` preserved / matches nothing after D1); D1 relation cleanup is implemented; automatic ArchiveItemPerson from PhotoPerson; staff PHOTO appearance review / PhotoPerson backfill from `people_present`; public Person filter/browse UX; no new-Person proposals
- Public Person pages
