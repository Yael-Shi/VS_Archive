...
- **Public `/archive/` search/filter:** full-text ``q`` remains authoritative for ranking. Structured advanced filters (`author`, repeatable `category`/`event`/`tag` ids, `year`/`year_to` overlap) compose on the same authorized queryset via ``archive_advanced_search.py`` (backend contract PR1; advanced UI deferred).
