...
- **Person identity:** **`Person`** is a minimal identified-person entity (canonical **`name`**, not unique, plus timestamps). **`PersonAlias`** stores alternate lookup/search names for that same identity (`unique(person, name)`; not globally unique; not a display name). **`ArchiveItemPerson`** links a person as generally related to an **`ArchiveItem`**. **`PhotoPerson`** links an identified person as appearing in a specific **`PhotoContent`**. Staff photo-edit can select existing people and create a name-only **`Person`**. The staff picker shows canonical name plus aliases (`Canonical (Alias 1, Alias 2)`); selected people have an **עריכת אדם** link to `/archive/manage/people/<id>/edit/` for canonical rename and alias add/edit/delete (PR6a services). There is no Person catalog or Admin. Public PHOTO detail shows canonical **`Person.name`** for the selected photo only; aliases are not publicly displayed and names are not links. There is no public Person page. `PhotoPerson` canonical names **and** aliases are searchable on the owning PHOTO ArchiveItem only when that Person is attached to a public-renderable photo; `ArchiveItemPerson` is not used for photo-appearance search and is **not** indexed for public `q`. Migration **`0055_backfill_person_from_person_name_tags`** copies 29 approved historical person-name Tags into `Person` + item-level `ArchiveItemPerson` only; old Tags remain for search/browse/filter; `PhotoPerson` and aliases are not inferred. Alias create/edit/delete uses explicit services with PhotoPerson index fan-out. Tags and `author_name` are unchanged. Person rename for search uses `update_person_name` (aliases are not rewritten). See **`decision-log.md`**.
