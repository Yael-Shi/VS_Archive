...
**Not implemented:** Gemini→Transkribus fallback, Transkribus→Gemini fallback, hybrid OCR routing, broader Transkribus routing beyond Hebrew handwritten, product/admin reprocess workflow, cleanup automation, general re-OCR on successful Trp runs, remote Trp deletion, paragraph staff UI / public rendering / copy-adopt writes, or automatic paragraph inheritance.
...
**Transkribus paragraph presentation metadata (PR1 — models/services only):** v1 is Transkribus-only. Staff-authored paragraph boundaries are presentation metadata on a `TranskribusTranscriptSnapshot` (`TranskribusParagraphMapping` + `TranskribusParagraphBreak` after contributing source lines). They do **not** modify `DocumentTextResult.text`, snapshot canonical text, snapshot lines, geometry, char offsets, or hover IDs. A mapping row with zero breaks is an explicit one-paragraph save; no mapping row means grouping was never saved. New snapshots do not inherit mappings; historical mappings remain stored. Currentness follows binding structural freshness + same snapshot (not `hover_eligible`). Cross-snapshot transfer is a fail-closed `(page_index, provider_line_id)` identity proof plus read-only historical suggestions; adoption UI is not implemented. `copied_from` is the source of the current saved division (set only by an explicit future adoption write; ordinary/manual resave clears it). Actor-less resave preserves `updated_by`.
