...
- **`PHOTO`:** **`ArchiveItem`** is the umbrella record; a PHOTO item may own **1..N** **`PhotoContent`** rows (FK `related_name="photo_contents"`). Existing items still have exactly one photo. Staff create via **`/archive/manage/new/?item_type=photo`** (PHOTO-specific upload API — **not** OCR **`/api/uploads/*`**). V1 UI/upload still creates and presents **one** photo per item (`ArchiveItem.primary_photo_content`). Includes direct S3 upload, metadata, **`public`** / **`private`** visibility, public archive list/detail display, and staff metadata edit/delete. **No** **`Document`**, OCR/HTR, or SQS. Multi-photo upload/gallery UI is not implemented. Thumbnail generation is implemented; S3 cleanup on delete is best-effort; re-upload/retry remain deferred. Design/scope: **`docs/ai-context/photo-archive-items.md`**. **`PhotoContent.people_present`** remains free-text; identified people use **`PhotoPerson`** (see Person identity below).
- **Person identity (schema foundation):** **`Person`** is a minimal identified-person entity (canonical **`name`**, not unique, plus timestamps). **`ArchiveItemPerson`** links a person as generally related to an **`ArchiveItem`**. **`PhotoPerson`** links an identified person as appearing in a specific **`PhotoContent`**. Tags, **`author_name`**, search, advanced search, and public UI are unchanged. See **`decision-log.md`**.
