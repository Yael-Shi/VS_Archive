...
- **`PHOTO`:** **`ArchiveItem`** is the umbrella record; a PHOTO item may own **1..N** **`PhotoContent`** rows (FK `related_name="photo_contents"`). Staff create via **`/archive/manage/new/?item_type=photo`**. Staff can add/edit/reorder/delete individual photos on the PHOTO manage page; a PHOTO item must keep at least one `PhotoContent`. Public list/detail still present the **first** photo (`ArchiveItem.primary_photo_content`). Includes direct S3 upload, metadata, **`public`** / **`private`** visibility, and staff metadata edit/delete. **No** **`Document`**, OCR/HTR, or SQS. Public multi-photo gallery and search aggregation across photos are not implemented. Thumbnail generation is implemented; S3 cleanup on delete is best-effort; re-upload/retry remain deferred. Design/scope: **`docs/ai-context/photo-archive-items.md`**. **`PhotoContent.people_present`** remains free-text; identified people use **`PhotoPerson`** (see Person identity below).
- **Person identity:** **`Person`** is a minimal identified-person entity (canonical **`name`**, not unique, plus timestamps). **`ArchiveItemPerson`** links a person as generally related to an **`ArchiveItem`**. **`PhotoPerson`** links an identified person as appearing in a specific **`PhotoContent`**. Staff photo-edit can select existing people and create a name-only **`Person`**. Tags, **`author_name`**, search, advanced search, and public UI are unchanged. See **`decision-log.md`**.
