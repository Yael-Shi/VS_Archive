...
still uses Tag-relation semantics (matches nothing after D1). D2 Tag-row
deletion is deferred (name resurrection + pending-suggestion policy).
No name denylist. Migration **`0055_backfill_person_from_person_name_tags`** copied 29 approved Tags into `Person` + item-level `ArchiveItemPerson` only; `PhotoPerson` and aliases were not inferred. Because `0055` predated ArchiveItemPerson search indexing, deploy of that search change requires **`backfill_archive_search_index`**. Alias create/edit/delete and `update_person_name` fan out to ArchiveItems linked through ArchiveItemPerson **and** PhotoPerson (one rebuild per item). Tags and `author_name` are unchanged. See **`decision-log.md`**.
