...
- **`Person.biography`** (staff-authored public Person summary; display-only on `/archive/people/<id>/`; names/aliases remain the only Person text in `metadata_text`)
...
| | |
|--|--|
| **Scope** | Repeatable `person=<person_id>` on public `/archive/` advanced search; match authorized+browse-renderable ArchiveItems via `ArchiveItemPerson` **or** identified renderable `PhotoPerson`; OR within Person, AND across other filter groups; canonical `Person.name` picker/chip labels; id form values; same conditional choice-context loading as category/event/tag |
| **Files** | `archive_advanced_search.py`; `person_public.py`; `archive_item_presentation.py`; `views.py` archive list; `archive/list.html`; pagination partial; tests; design + decision-log + context |
| **Migrations** | None |
| **Non-goals** | Per-photo search result rows; changing OR-within-Person to AND; alias-assisted picker search; treating `people_present` as identity; inferring ArchiveItemPerson from PhotoPerson; removing historical person-name Tags; `q` index changes; schema changes. Person filter-chip UX and public Person browse/detail are implemented separately. Photo-aware ArchiveItem **card** presentation under this filter is implemented separately (see next row).

### Photo-aware presentation for advanced Person filter — **implemented**

| | |
|--|--|
| **Scope** | After ArchiveItem-level `person=` filtering, PHOTO browse cards on the page slice may deep-link to `/archive/<item_id>/?photo=<photo_id>` and use that photo’s thumbnail when there is one truthful matching renderable PhotoContent: one selected Person → earliest matching PhotoPerson appearance (including AIP+PP); several selected People → a photo containing all selected ids, else ordinary item URL/primary thumbnail |
| **Files** | `person_public.py` (`matching_photo_ids_for_selected_persons`, `apply_matching_photo_presentation_to_cards`); `views.py` `archive_list_page`; tests; decision-log + context |
| **Migrations** | None |
| **Non-goals** | Changing filter membership; per-photo result rows; AIP/PhotoPerson badges; clickable thumbnails; homepage/discovery-browse overrides; Person-detail membership (AIA) on the list path |

### Public Person active-filter chip UX — **implemented**

| | |
|--|--|
| **Scope** | One `/archive/` active chip per selected `person=` id; canonical name GET-links to `archive-person-detail`; visible × GET-link removes only that id; preserve `q` / other filters / `item_type` / `per_page`; drop `page`; always keep `advanced=1` |
| **Files** | `archive_item_presentation.py`; `archive/list.html`; `app.css`; tests; decision-log + context |
| **Migrations** | None |
| **Non-goals** | Changing non-Person chips; PhotoPerson; aliases; Person pages; D2; filtering/pagination semantics elsewhere |
