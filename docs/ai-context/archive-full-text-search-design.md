...
| **Migrations** | None |
| **Non-goals** | Photo-level appearance cards/filter; alias-assisted picker search; treating `people_present` as identity; inferring ArchiveItemPerson from PhotoPerson; removing historical person-name Tags; `q` index changes; schema changes. Person filter-chip UX and public Person browse/detail are implemented separately.
